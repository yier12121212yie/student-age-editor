...
**独立工具版本**：从 backend/editor/services/unityfs_res.py 整体剥离（W5-3，
波次 4 删除 backend/editor 后本文件为唯一出处）。`UnityFsIndex` 的全部行为
语义与后端原件逐字一致（它是被 C++ aa 端点对齐过的真相源），唯一差异是
`detect_game_aa_dir` 的实现不再依赖 `editor.core.steam_paths`——改为本文件
内联的标准 Steam 库枚举 + `SA_GAME_AA_DIR` 环境变量覆盖，简化点见该函数
注释。
...
# ------------------------------------------------------------- 游戏目录探测 --
# 内联自 backend/editor/core/steam_paths.py 的 detect_game_aa_dir（W5-3 剥离）。
#
# 与后端原实现的等价范围：标准 Steam 安装根（Windows 注册表 SteamPath +
# ProgramFiles 回退；Linux .steam/.local/share/Flatpak；macOS Application
# Support）→ 解析 libraryfolders.vdf 发现全部 Steam 库 → 拼
# <库>/steamapps/common/StudentAge/StudentAge_Data/StreamingAssets/aa/<平台>。
#
# 有意简化的点（游戏本体只有 Windows 版，本工具用于桌面构建期，够用）：
# - 不实现 Proton/CrossOver bottle 路径探测（backend 的 proton_mods_dirs），
#   因为那与「找游戏数据」正交；Linux 上游戏数据仍按 Steam 库路径直接找。
# - 不读 editor_env.json / 不依赖 editor.core.paths 的数据目录回退值。
# - 提供 SA_GAME_AA_DIR 环境变量直接指定 Addressables 根目录，作为上述
#   枚举失败时的显式逃生口（测试/非标准安装用）。
...
def detect_game_aa_dir():
    ...
    优先返回 SA_GAME_AA_DIR（显式覆盖；设置但目录不存在则返回空串，避免
    静默改用别的路径）。否则按 Steam 库枚举 + AA_PLATFORM_DIRS 优先级查找。
    """
    override = (os.environ.get("SA_GAME_AA_DIR") or "").strip()
    if override:

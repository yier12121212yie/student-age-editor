...
**独立工具版本**：从 backend/editor/services/decoded_pack.py 原样剥离（W5-3，
波次 4 删除 backend/editor 后本文件为唯一出处）。它是**消费侧**实现——与
`decoded_export.py`（生产侧）构成闭环：export 写出 zip，本类在运行时扫描
解压后的目录把 key 映射到磁盘文件。

背景：Android（Chaquopy）无游戏目录且缺 ASTC/FSB 原生解码器，桌面管线
`tools/resource_scan/decoded_export.py` 在构建期把纹理转 WebP、音频/文本原样
导出为资源包（aa_index.json v3 携带 tex/aud/txt 键列表 + tex/*.webp、aud/*.ogg）。

// sa_core/sha256.cpp: FIPS 180-4 SHA-256, single-shot over an in-memory
// byte string (revision_manager hashes whole files; streaming is not needed).
...
std::string sha256_hex(std::string_view data) {
    ...
    // FIPS 180-4 padding: 0x80, zeros to 56 mod 64, big-endian 64-bit bit length.
    ...
    for (int i = 0; i < 8; ++i)
        for (int shift = 24; shift >= 0; shift -= 8) {
            out.push_back(kHex[(h[i] >> (shift + 4)) & 0xf]);

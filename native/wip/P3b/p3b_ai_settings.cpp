// wip/P3b — see p3b_ai_settings.h (port of env_store.py AI half).
#include "p3b_ai_settings.h"

#include <algorithm>
#include <cctype>
#include <cmath>
#include <cstdio>
#include <cstdlib>
#include <map>
#include <mutex>
#include <optional>
#include <vector>

#include "p3b_support.h"
#include "sa_core/atomic_io.h"
#include "sa_core/env_store.h"
#include "sa_core/json_wire.h"
#include "sa_core/paths.h"
#include "sa_core/strings.h"
#include "sa_core/util.h"

namespace sa {
namespace p3b {
namespace ai_settings {
namespace {

namespace cs = sa_core::paths;

std::mutex g_write_lock;  // env_store._WRITE_LOCK (read-merge-write atomicity)

bool is_provider(const std::string& s) {
    return s == "openai_compatible" || s == "openai_responses" || s == "anthropic";
}
bool is_permission_mode(const std::string& s) { return s == "confirm" || s == "full"; }

std::string ascii_lower(std::string s) {
    for (auto& c : s) c = static_cast<char>(std::tolower(static_cast<unsigned char>(c)));
    return s;
}

// float(x) on the JSON scalars normalize can hand it (bools never arrive —
// they are skipped upstream; containers -> TypeError -> nullopt).
std::optional<double> to_float(const json& v) {
    if (v.is_number()) return v.get<double>();
    if (v.is_string()) return py_float_str(v.get<std::string>());
    return std::nullopt;
}

// int(x): float truncates toward zero; string strict.
std::optional<long long> to_int(const json& v) {
    if (v.is_number_integer()) return v.get<long long>();
    if (v.is_number_unsigned()) return static_cast<long long>(v.get<unsigned long long>());
    if (v.is_number_float()) return static_cast<long long>(v.get<double>());
    if (v.is_string()) return sa_core::py_int(v.get<std::string>());
    return std::nullopt;
}

// round(x, 2): CPython uses round-half-even on the exact binary value; the
// CRT %.2f is correctly rounded, which reproduces it for clamped inputs.
double round2(double x) {
    char buf[32];
    std::snprintf(buf, sizeof(buf), "%.2f", x);
    return std::strtod(buf, nullptr);
}

double clampd(double v, double lo, double hi) { return v < lo ? lo : (v > hi ? hi : v); }
long long clampi(long long v, long long lo, long long hi) {
    return v < lo ? lo : (v > hi ? hi : v);
}

// env_store.normalize_ai_settings alias table.
const std::vector<std::string>& aliases_for(const std::string& key) {
    static const std::map<std::string, std::vector<std::string>> kAlt = {
        {"baseUrl", {"base_url"}},
        {"apiKey", {"api_key", "apikey"}},
        {"imageModel", {"image_model"}},
        {"imageApiKey", {"image_api_key"}},
        {"imageBaseUrl", {"image_base_url"}},
        {"temperature", {"temp"}},
        {"ttsProvider", {"tts_provider"}},
        {"ttsApiKey", {"tts_api_key"}},
        {"ttsBaseUrl", {"tts_base_url"}},
        {"ttsModel", {"tts_model"}},
        {"ttsVoice", {"tts_voice"}},
        {"ttsGroupId", {"tts_group_id"}},
        {"ttsSpeed", {"tts_speed"}},
        {"ttsVolume", {"tts_volume"}},
        {"ttsPitch", {"tts_pitch"}},
        {"ttsFormat", {"tts_format"}},
        {"permissionMode", {"permission_mode"}},
    };
    static const std::vector<std::string> kNone;
    auto it = kAlt.find(key);
    return it == kAlt.end() ? kNone : it->second;
}

const std::vector<std::string>& default_keys() {
    static const std::vector<std::string> kKeys = [] {
        std::vector<std::string> v;
        for (auto it = defaults().begin(); it != defaults().end(); ++it) v.push_back(it.key());
        return v;
    }();
    return kKeys;
}

}  // namespace

const json& defaults() {
    static const json kDefaults = [] {
        json d = json::object();
        d["provider"] = "openai_compatible";
        d["baseUrl"] = "";
        d["apiKey"] = "";
        d["model"] = "";
        d["temperature"] = 0.7;
        d["imageModel"] = "";
        d["imageApiKey"] = "";
        d["imageBaseUrl"] = "";
        d["ttsProvider"] = "";
        d["ttsApiKey"] = "";
        d["ttsBaseUrl"] = "";
        d["ttsModel"] = "";
        d["ttsVoice"] = "";
        d["ttsGroupId"] = "";
        d["ttsSpeed"] = 1.0;
        d["ttsVolume"] = 1.0;
        d["ttsPitch"] = 0;
        d["ttsFormat"] = "wav";
        d["maxRetries"] = 3;
        d["retryDelayMs"] = 1000;
        d["permissionMode"] = "confirm";
        return d;
    }();
    return kDefaults;
}

json normalize(const json& data) {
    json out = defaults();
    if (!data.is_object()) return out;
    for (const auto& key : default_keys()) {
        json val = data.contains(key) ? data.at(key) : json();
        if (val.is_null()) {
            for (const auto& a : aliases_for(key)) {
                if (data.contains(a) && !data.at(a).is_null()) {
                    val = data.at(a);
                    break;
                }
            }
        }
        if (val.is_string()) {
            out[key] = py_strip(val.get<std::string>());
        } else if (val.is_boolean()) {
            continue;  // no boolean field; keep the default
        } else {
            out[key] = val;  // null / number / container passthrough — this is
                             // the path that pins apiKey/baseUrl/model to null
                             // on a fresh store (golden api_ai_settings.json).
        }
    }
    if (!out["provider"].is_string() || !is_provider(out["provider"].get<std::string>())) {
        out["provider"] = "openai_compatible";
    }
    // temperature: 0.0 is legal — never `or 0.7` (env_store.py:157-158).
    double temp = 0.7;
    if (!out["temperature"].is_null()) {
        auto f = to_float(out["temperature"]);
        temp = f.has_value() ? *f : 0.7;
    }
    out["temperature"] = round2(clampd(temp, 0.0, 2.0));
    for (const char* k : {"imageModel", "imageApiKey", "imageBaseUrl"}) {
        if (!out[k].is_string()) out[k] = "";
    }
    // ---- tts block (env_store.py:172-209) ----
    if (out["ttsProvider"].is_string()) {
        out["ttsProvider"] = ascii_lower(out["ttsProvider"].get<std::string>());
    }
    // Python membership on the raw value: None not in ("", "minimax",
    // "aliyun") is TRUE -> reset to "". So a non-string (e.g. absent -> null)
    // must become "", NOT be treated as ""-valid.
    const json& tpv = out["ttsProvider"];
    const bool tp_ok = tpv.is_string() &&
                       (tpv.get_ref<const std::string&>() == "" ||
                        tpv.get_ref<const std::string&>() == "minimax" ||
                        tpv.get_ref<const std::string&>() == "aliyun");
    if (!tp_ok) out["ttsProvider"] = "";
    for (const char* k :
         {"ttsApiKey", "ttsBaseUrl", "ttsModel", "ttsVoice", "ttsGroupId", "ttsFormat"}) {
        if (!out[k].is_string()) out[k] = "";
    }
    if (out["ttsFormat"].get_ref<const std::string&>().empty()) out["ttsFormat"] = "wav";
    double speed = 1.0;
    if (!out["ttsSpeed"].is_null()) {
        auto f = to_float(out["ttsSpeed"]);
        speed = f.has_value() ? *f : 1.0;
    }
    out["ttsSpeed"] = round2(clampd(speed, 0.5, 2.0));
    double vol = 1.0;
    if (!out["ttsVolume"].is_null()) {
        auto f = to_float(out["ttsVolume"]);
        vol = f.has_value() ? *f : 1.0;
    }
    out["ttsVolume"] = round2(clampd(vol, 0.5, 2.0));
    long long pitch = 0;
    if (json_truthy(out["ttsPitch"])) {  // `int(x or 0)`
        auto iv = to_int(out["ttsPitch"]);
        pitch = iv.has_value() ? *iv : 0;
    }
    out["ttsPitch"] = clampi(pitch, -12, 12);
    long long retries = 3;
    if (!out["maxRetries"].is_null()) {
        auto iv = to_int(out["maxRetries"]);
        retries = iv.has_value() ? *iv : 3;
    }
    out["maxRetries"] = clampi(retries, 0, 10);
    long long delay = 1000;
    if (!out["retryDelayMs"].is_null()) {
        auto iv = to_int(out["retryDelayMs"]);
        delay = iv.has_value() ? *iv : 1000;
    }
    out["retryDelayMs"] = clampi(delay, 0, 30000);
    std::string pm;
    if (out["permissionMode"].is_string()) {
        pm = ascii_lower(py_strip(out["permissionMode"].get<std::string>()));
    }
    if (!is_permission_mode(pm)) pm = "confirm";
    out["permissionMode"] = pm;
    return out;
}

std::string path_for(const std::string& editor_root) {
    return cs::join(editor_root, ".editor_ai.json");
}

json read_settings(const std::string& editor_root) {
    return normalize(sa_core::env_store::read_json_file(path_for(editor_root)));
}

json write_settings(const std::string& editor_root, const json& patch) {
    std::lock_guard<std::mutex> lk(g_write_lock);
    json current = read_settings(editor_root);
    json merged_raw = current;
    if (patch.is_object()) {
        for (auto it = patch.begin(); it != patch.end(); ++it) {
            if (!it.value().is_null()) merged_raw[it.key()] = it.value();
        }
    }
    json normalized = normalize(merged_raw);
    // _merge_write_unlocked: merge over the RAW file content (unknown extra
    // keys survive), atomically rewrite with indent=2.
    const std::string p = path_for(editor_root);
    json data = sa_core::env_store::read_json_file(p);
    json merged = data;
    for (auto it = normalized.begin(); it != normalized.end(); ++it) merged[it.key()] = it.value();
    sa_core::write_text_atomic(p, sa_core::py_dumps_indent(merged));
    return normalized;
}

}  // namespace ai_settings
}  // namespace p3b
}  // namespace sa

...
void register_p4_base_store() {
    ...
    // NO eager load: Python keeps STATE.base idle until POST /api/base/load
    // (api.py:2389-2393 -> base_service.load_async; preview/base_ids all read
    // the idle store). An eager 38MB parse also stalled bind() by >20s when
    // Defender cold-scanned the artifact dir (wave-2 integration finding).
}
